...
class Adam(Optimizer):
    ...
    def get_updates(self, params, constraints, loss):
        ...
        for p, g, c in zip(params, grads, constraints):
            m = theano.shared(p.get_value() * np.cast[p.get_value().dtype](0.))  # zero init of moment
            v = theano.shared(p.get_value() * np.cast[p.get_value().dtype](0.))  # zero init of velocity
